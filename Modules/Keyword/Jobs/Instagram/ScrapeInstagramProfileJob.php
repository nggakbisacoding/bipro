...
class ScrapeInstagramProfileJob implements ShouldQueue
{
    ...
    {
        $lastPostDate = $instagramService->getLatestPostFromProfile($this->username);
        Keyword::whereName($this->username)
            ->whereSource(Keyword::SOURCE_INSTAGRAM)
            ->first()
            ->update([
                'last_post' => $lastPostDate ?? null,
            ]);
    }
}
